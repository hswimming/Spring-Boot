...
// (aspect 실제 구현체) 
// * 동작시점 : 조인 포인트 실행 전/후로 before, after, after returning, after throwing, around로 구분 

@Component // <context:component-scan base-package="com.shinhan"/>
@Aspect // <aop:aspectj-autoproxy/>
@Order(2) // 수행 순서 (낮은것이 먼저)
public class LoggingAdvice  {
	...
	public Object aroundMethod2(ProceedingJoinPoint jp) throws Throwable {
		...
		// 1.around
		// 2.before
		System.out.println("[around] ====== LoggingAdvice 보조업무 ======");
		System.out.println(jp.getSignature().getName() + " 메서드 호출 전 [around]");
		// 3.주업무
		...
		// 5.after
		// 6.around
		System.out.println(jp.getSignature().getName() + " 메서드 호출 후 [around]");
